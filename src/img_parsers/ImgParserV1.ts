...
export class ImgParserV1 extends ImgParserBase {
  public parse(colorBoardIndex: number = 0): ImgFrame[] {
    ...
    for (let i = 0; i < this.indexCount; i++) {
      ...
      if (entry.type === 'link') {
        ...
        let actualDataSize = entry.size!;
        if (entry.comp === 5) {
          let bpp = 4;
          if (entry.color === 14 || entry.color === 15) bpp = 2;
          actualDataSize = entry.w * entry.h * bpp;
        }
        
        entry.size = actualDataSize;
        entry.rawDataOffset = pos;
        entries.push(entry);
        pos += actualDataSize;
      }
    }
    ...
    for (let i = 0; i < this.indexCount; i++) {
      ...
      if (entry.size! > 0) {
        if (entry.rawDataOffset! + entry.size! > this.buffer.byteLength) {
          console.warn(`[ImgParserV1] Frame ${i} raw data offset exceeds buffer bounds, skipping.`);
          continue;
        }
        rawData = new Uint8Array(this.buffer, entry.rawDataOffset!, entry.size!);
      }
